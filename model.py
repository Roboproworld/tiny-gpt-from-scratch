...
def slice_row(arr, i):
    ...
    return np.array([arr[i][j] for j in range(len(arr[0])) ])
...
def slice_column(arr, j):
    ...
    return np.array([arr[i][j] for i in range(len(arr))])
...
def softmax_overflow_demo(large_value):
    ...
    if np.exp(large_value) < float('inf'):
        return {
...
def stable_softmax_1d(logits):
    ...
    logits = logits- max_along_axis(logits, axis= 0)
    return array_exp(logits)*sum_all(array_exp(logits))**-1
...
def stable_softmax_2d_rowwise(logits):
    ...
    for i in range(len(logits)):
        logits[i] = logits[i]- max_along_axis(logits[i],axis= 0)
        logits[i] = array_exp(logits[i])*sum_keepdims(array_exp(logits[i]),axis= 0)**-1
    return logits
...
def stack_x_batch(data, offsets, block_size):
    ...
    for offset in offsets:
        matrix.append(slice_x_at_offset(data,offset,block_size))
...
def vectorize_counts_add_at(vocab_size, data):
    ...
    vocab_matrix = allocate_count_matrix(vocab_size)
    np.add.at(vocab_matrix,(data[:-1],data[1:]),1)
    return vocab_matrix
...
def normalize_counts_to_probs(n_matrix):
    ...
    return n_matrix*row_sums_of_counts(n_matrix)**-1
...
def sample_next_token(p_matrix, current_id, rng):
    ...
    return int(rng.choice(range(len(p_matrix[0])),p= p_matrix[current_id]))
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    for t in range(1,length):
        sequence.append(sample_next_token(p_matrix,sequence[t-1],rng))
...
def sum_negative_log_probs(p_matrix, data):
    ...
    return (np.sum([log_prob_of_pair(p_matrix,data[t],data[t+1]) for t in range(len(data)-1) ]))*-1

# Step 56 - average_nll
def average_nll(p_matrix, data):
    # TODO: return mean negative log likelihood per bigram over consecutive pairs in data.
    return sum_negative_log_probs(p_matrix,data)*(len(data)-1)**-1
...
def initialize_w_random(vocab_size, rng):
    ...
    return rng.standard_normal(size= (vocab_size,vocab_size),dtype= float)
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    ohe_matrix = make_2d_zeros(len(ids),vocab_size)
...
def observe_lookup_equivalence(w, ids):
    ...
    return {
        "onehot_result": matmul(one_hot_encode_batch(ids,len(w)),w),
        "index_result": np.array([w[id_] for id_ in ids])
    }
...
def forward_logits_lookup(w, ids):
    ...
    return np.array([w[id_] for id_ in ids])
...
def gather_correct_token_probs(probs, targets):
    ...
    return np.array([probs[i,target] for i,target in enumerate(targets)])
...
def cross_entropy_loss(probs, targets):
    ...
    return np.average(array_log(gather_correct_token_probs(probs,targets))*-1)
